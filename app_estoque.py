...
# Colunas que precisam de limpeza e conversão numérica (todas as colunas numéricas envolvidas)
COLUNAS_NUMERICAS_LIMPEZA = ['Em_Estoque', ' Media de venda semanal']

# --- Funções de Formatação (Padrão Brasileiro) ---

# Função lambda para formatar números (milhar ponto, decimal vírgula - ex: 1.234,50)
def formatar_br_numero(x):
    if pd.isna(x):
        return ''

    # Decide se formata como inteiro ou com duas casas decimais
    # Usa duas casas se não for um número "redondo" (inteiro)
    s = f"{x:,.2f}" if x % 1 != 0 or x == 0 else f"{int(x):,}"

    # Inverte os separadores: vírgula milhar -> ponto, ponto decimal -> vírgula
    # Esta é a correção robusta que funciona independente da configuração do sistema operacional
...
if not df_estoque.empty:
    ...
    for col_filtro in ['Analise de estoque', 'Grupo_de_Estoque']:
        if col_filtro in df_estoque.columns:
            # Substitui NaN por uma string para evitar erro no filtro de combobox
    ...
    try:
        ...
        # CORREÇÃO DO ERRO 'return outside function'
    ...
    for col in COLUNAS_NUMERICAS_LIMPEZA:
        if col in df_display.columns:
            # Aplica a formatação de números (milhar ponto, decimal vírgula)
